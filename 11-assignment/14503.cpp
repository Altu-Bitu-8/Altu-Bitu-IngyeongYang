...
const int SIZE = 50; //보드 최대 크기
const int CLEAN = 2; //청소 완료 값
...
void dfs(int row, int col, int dir) {
  // 1. 현재 위치 청소
  if(board[row][col] != CLEAN) { //청소가 안 된 경우
    cnt++; //청소한 칸 수 증가
  }
  board[row][col] = CLEAN; //청소 완료 칸으로 표시
  ...
  for(int i = 0; i < 4; i++) { // 3-1. 반시계 방향으로 90º 회전
    ...
    if(board[new_row][new_col] == 0) { // 3-2. 아직 청소되지 않은 빈 칸 발견
      ...
      return; //이동 후 함수에서 빠져나가기
    }
  }

  // 2. 현재 칸의 주변 4칸 중 청소되지 않은 빈 칸이 없는 경우
  int back_dir = (dir+2) % 4; //후진하기
